...
//
// Reuses the repo identity derived from the `origin` remote (room.ts). We only
// know how to talk to github.com, so non-GitHub remotes report
// `provider: 'git'` and the renderer shows a "not a GitHub repo" message
// instead of calling here.
//
// The GitHub OAuth token is injected by the host (createPullRequests) so this
// client stays free of auth-storage / Electron coupling.

import { getRoomIdentity } from './room'

const UA = 'codeswim'

// The host wires in the token getter (from createGitHubAuth().getToken).
export interface PullRequestClientOptions {
...
export function createPullRequestClient(options: PullRequestClientOptions): PullRequestClient {
  ...
  async function resolveRepo(
    rootPath: string
  ): Promise<{ ownerRepo: string; slug: string; token: string | null } | null> {
  ...
  // `.diff` media type on the pull endpoint. We fetch it here (rather than
  // leaning on the agent's shell) so the review works without `gh` configured.
  async function pullRequestDiff(rootPath: string, number: number): Promise<PullRequestDiff> {
    const repo = await resolveRepo(rootPath)
  ...
  // to the repo. GitHub enforces mergeability (conflicts, required checks,
  // branch protection) server-side; we surface its refusal as 'blocked' with
  // the message it returns rather than guessing client-side.
  ...
  return { listPullRequests, pullRequestDiff, mergePullRequest }
}
