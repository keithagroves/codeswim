...
// The host (apps/desktop) knows where the built-in skill prompts live on disk
// (the asar-unpacked layout differs packaged vs. dev), so it injects the
// resolved absolute path once at startup. Module state is fine: the main
// process wires this a single time.
let builtinSkillsDir: string | null = null

export function configureBuiltinSkillsDir(dir: string): void {
...
function builtinDir(): string {
  if (!builtinSkillsDir) {
    throw new Error(
      'skills: built-in skills directory not configured; call configureBuiltinSkillsDir() at startup'
    )
  }
