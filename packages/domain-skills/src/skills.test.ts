...
//
// The production module no longer imports Electron: the host injects the
// built-in prompt path via configureBuiltinSkillsDir(). Tests cover the
// workspace + symlink behavior, so we point it at a dir under cwd.
...
skills.configureBuiltinSkillsDir(path.join(process.cwd(), 'out', 'harness', 'prompt'))
