...
| **Diagram/source coverage (Sync audit)** | [CoveragePanel.tsx](apps/desktop/src/renderer/src/components/CoveragePanel.tsx), [coverage/run.ts](apps/desktop/src/renderer/src/coverage/run.ts), [coverage](packages/coverage/src/coverage.ts) | Left panel (`coverage` section); "Fix with agent" routes to agent chat; also invoked from Commit panel / toast | Built | Persistent section + On-demand → feeds Agent | No — has its own section now (Commit-panel entry point kept) |
