...
- [apps/desktop/src/main/kanban-worktree.ts](../apps/desktop/src/main/kanban-worktree.ts) — per-card git worktrees for Kanban "Run all": isolated checkouts under userData (keyed by a hash of the workspace root) so parallel agent runs can't stomp each other's edits, and never show up in the watched tree. `listCardWorktrees` enumerates them with their uncommitted change counts so the Sync panel can offer each card branch as a sync target.
